...
const React360Viewer = /* #__PURE__ */ (function (_Component) {
  _inherits(React360Viewer, _Component);

  const _super = _createSuper(React360Viewer);
  ...
  function React360Viewer() {
    ...
    _defineProperty(
      ...
      function (evt) {
        ...
        _this.dragStart = _this.ctx.transformedPoint(
          _this.state.lastX,
          _this.state.lastY
        );
      }
    ...
    _defineProperty(
      ...
      function (evt) {
        ...
        if (_this.dragStart) {
          const pt = _this.ctx.transformedPoint(
            _this.state.lastX,
            _this.state.lastY
          );

          _this.ctx.translate(
            pt.x - _this.dragStart.x,
            pt.y - _this.dragStart.y
          ); // redraw();
  ...
  _createClass(React360Viewer, [
    ...
    },
    {
      ...
      value: function fetchData() {
        this.props.listImage360.forEach((item) => {
      ...
      value: function preloadImages() {
        ...
        if (this.imageData.length) {
          try {
            ...
            console.error(
              'Something went wrong while loading images: '.concat(
                error.message
              )
            );
          }
    ...
    },
    {
      ...
      value: function bindPanModeEvents() {
        if (this.viewPortElementRef) {
          ...
          this.viewPortElementRef.addEventListener(
            'touchend',
            this.stopDragging
          );
          this.viewPortElementRef.addEventListener(
      ...
      value: function setImage() {
        ...
        if (!cached) {
          ...
          this.currentCanvasImage.onload = function () {
            if (_this3.viewPortElementRef) {
              const viewportElement =
                _this3.viewPortElementRef.getBoundingClientRect();

              _this3.canvas.width = _this3.state.isFullScreen
                ? viewportElement.width
                : _this3.currentCanvasImage.width;
              _this3.canvas.height = _this3.state.isFullScreen
                ? viewportElement.height
                : _this3.currentCanvasImage.height;
    ...
    },
    {
      ...
      value: function zoom(clicks) {
        ...
        const factor = Math.pow(1.01, clicks); // console.log(factor)

        if (factor > 1) {
          this.currentScale += factor;
        } else if (this.currentScale - factor > 1) this.currentScale -= factor;
